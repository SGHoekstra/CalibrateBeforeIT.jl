...
@testset "Parameter Constraints Tests" begin
    ...
    if isfile(reference_file)
        ...
        @testset "Behavioral Parameters - Dividend Payout" begin
            # theta_DIV (dividend payout ratio) should be positive and reasonable.
            # Values slightly > 1 are plausible: firms can distribute from retained
            # earnings/reserves, especially in specific years.
            @test haskey(params, "theta_DIV")
            theta_DIV = params["theta_DIV"]
            @testset "theta_DIV positive (got $theta_DIV)" begin
                @test theta_DIV > 0
            end
            @testset "theta_DIV < 1.5 (got $theta_DIV)" begin
                @test theta_DIV < 1.5
            end
        end
